...
class Line():
    ...
    def add_fit(self, limg, rimg, mx=3.75/900, my=20/700):
        ...
        # Otherwise, Calculate the parameters. 
            ...
            self.lcurrent_fit = np.polyfit(ly_points*my, lx_points*mx, 4)
            ...
            self.lradius_of_curvature = ((1 + (4 * self.lcurrent_fit[0] * ly_base ** 3 + 3 * self.lcurrent_fit[
                1] * ly_base ** 2 + 2 * self.lcurrent_fit[2] * ly_base + self.lcurrent_fit[
                                                   3]) ** 2) ** 1.5) / (
                                                    12 * self.lcurrent_fit[0] * ly_base ** 2 + 6 *
                                                    self.lcurrent_fit[1] * ly_base + 2 * self.lcurrent_fit[2])
            # Calculate the horizontal location of the base point
            self.lline_base_pos = self.lcurrent_fit[0] * ly_base ** 4 + self.lcurrent_fit[1] * ly_base ** 3 + \
                                  self.lcurrent_fit[2] * ly_base ** 2 + self.lcurrent_fit[3] * ly_base + \
                                  self.lcurrent_fit[4] - mx * np.float(self.limg.shape[1]) / 2
            self.lfit = self.lcurrent_fit

            # Calculate the parameters of the right lane
            self.rdetected = True
            [ry_points, rx_points] = np.nonzero(self.rimg)
            self.rcurrent_fit = np.polyfit(ry_points*my, rx_points*mx, 4)
            self.ry_base = my*np.float(self.rimg.shape[0])
            ry_base = my*self.rimg.shape[0]
            self.rradius_of_curvature = ((1 + (4 * self.rcurrent_fit[0] * ry_base ** 3 + 3 * self.rcurrent_fit[
                1] * ry_base ** 2 + 2 * self.rcurrent_fit[2] * ry_base + self.rcurrent_fit[
                                                   3]) ** 2) ** 1.5) / (
                                                    12 * self.rcurrent_fit[0] * ry_base ** 2 + 6 *
                                                    self.rcurrent_fit[1] * ry_base + 2 * self.rcurrent_fit[2])
            self.rline_base_pos = self.rcurrent_fit[0] * ry_base ** 4 + self.rcurrent_fit[1] * ry_base ** 3 + \
                                  self.rcurrent_fit[2] * ry_base ** 2 + self.rcurrent_fit[3] * ry_base + \
                                  self.rcurrent_fit[4] - mx * np.float(self.rimg.shape[1]) / 2
    ...
    def best_fit(self, mx=3.75/900, my=20/700):
        ...
        self.lradius_of_curvature = ((1 + (4 * self.lcurrent_fit[0] * self.ly_base ** 3 + 3 * self.lcurrent_fit[1] * self.ly_base ** 2 + 2 * self.lcurrent_fit[2] * self.ly_base + self.lcurrent_fit[3]) ** 2) ** 1.5) / ( 12 * self.lcurrent_fit[0] * self.ly_base ** 2 + 6 * self.lcurrent_fit[1] * self.ly_base + 2 * self.lcurrent_fit[2])
        self.lline_base_pos = self.lcurrent_fit[0] * self.ly_base ** 4 + self.lcurrent_fit[1] * self.ly_base ** 3 + self.lcurrent_fit[2] * self.ly_base** 2 + self.lcurrent_fit[3] * self.ly_base + self.lcurrent_fit[4] - mx * np.float(self.limg.shape[1]) / 2
        self.rradius_of_curvature = ((1 + (4 * self.rcurrent_fit[0] * self.ry_base ** 3 + 3 * self.rcurrent_fit[1] * self.ry_base ** 2 + 2 * self.rcurrent_fit[2] * self.ry_base + self.rcurrent_fit[3]) ** 2) ** 1.5) / ( 12 * self.rcurrent_fit[0] * self.ry_base ** 2 + 6 * self.rcurrent_fit[1] * self.ry_base + 2 * self.rcurrent_fit[2])
        self.rline_base_pos = self.rcurrent_fit[0] * self.ry_base ** 4 + self.rcurrent_fit[1] * self.ry_base ** 3 + self.rcurrent_fit[2] * self.ry_base ** 2 + self.rcurrent_fit[3] * self.ry_base + self.rcurrent_fit[4] - mx * np.float(self.rimg.shape[1]) / 2
    ...
    def lane_region(self, mx=3.75/900, my=20/700, src=src, dst=dst):
        ...
        lx_at_y_grid = self.lfit[0] * (y_grid * my) ** 4 + self.lfit[1] * (y_grid * my) **3 +self.lfit[2] * (y_grid * my) **2 + self.lfit[3]*(y_grid * my) + self.lfit[4]
        rx_at_y_grid = self.rfit[0] * (y_grid * my) ** 4 + self.rfit[1] * (y_grid * my) **3 +self.rfit[2] * (y_grid * my) **2 + self.rfit[3]*(y_grid * my) + self.lfit[4]
